 ...
 */
public class ItemTierDown extends ItemTierSystem
{

    public ItemTierDown()
    {
        super();
        this.setUnlocalizedName("tierDown");
        ...
        if(ConfigCore.tierCount != 0)
        {
            if(!world.isRemote)
            {
                if(TierHandler.getCurrentTierForPlayer(entityPlayer) < ConfigCore.tierCount)
                {
                    ...
                    {
                        MinecraftServer.getServer().getConfigurationManager().sendChatMsg(new ChatComponentText(StatCollector.translateToLocal("tierDown1") + " " + TierHandler.getCurrentTierForPlayer(entityPlayer) + "."));
                        TierSystem.pipeline.sendToAll(new PacketSyncTiers());
                    } else
                    {
                        entityPlayer.addChatComponentMessage(new ChatComponentText(StatCollector.translateToLocal("tierDown1") + " " + TierHandler.getCurrentTierForPlayer(entityPlayer) + "."));
